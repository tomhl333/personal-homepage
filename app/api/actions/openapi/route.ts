...
const commonParameters = [
  ...
  { name: "author", in: "query", description: "Optional book author. Omit it when unknown; the server automatically looks it up from the matched book source.", schema: { type: "string" } },
  { name: "creator", in: "query", schema: { type: "string" } },
  { name: "platform", in: "query", description: "Optional release or streaming platform. Omit it when unknown; the server automatically looks it up from TMDB or the matched media source.", schema: { type: "string" } },
  { name: "mediaKind", in: "query", description: "Optional media type. Omit it when unknown; the server automatically distinguishes TV series, films, documentaries, and variety shows.", schema: { type: "string", enum: ["电视剧", "电影", "纪录片", "综艺"] } },
