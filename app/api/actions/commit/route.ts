...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!body.input) return NextResponse.json({ ok: false, message: "missing_input" }, { status: 400 });
    const result = await commitAction({ authorization: internalApiAuthorization(request.headers.get("authorization") ?? ""), confirmed: body.confirmed === true, input: body.input, origin: request.nextUrl.origin, targetId: body.targetId });
