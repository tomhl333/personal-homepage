...
For requests such as today's training plan, call the personal-homepage `previewTrainingPlan` Action. It delegates to the existing Xunheng planner, which uses training history, Apple Health recovery data, sleep/HRV, recent load, equipment, time, and optional target focus. Required inputs are `equipmentMode` (`dumbbell`, `free_weights`, or `gym`), `sessionMinutes` (30/45/60/75/90), and `subjectiveState` (`tired`, `normal`, or `good`); use `targetFocus=auto` unless the user explicitly chooses chest, back, shoulders, legs, or arms. Ask for missing inputs instead of guessing. The user's standing preference is to avoid movements performed prone, supine, or seated on the floor—including hip thrusts, prone dumbbell leg curls, and floor-based ab work. Prefer standing, seated, machine, or bench-supported alternatives and do not ask about this preference again.
